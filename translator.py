...
class XLSXTranslator:
    ...
    @staticmethod
    def adjust_formula_sheet_references(
        formula: str, sheet_name_map: Dict[str, str]
    ) -> str:
        """
        ...
        def repl(match):
            ...
            sheet_unquoted = (
                sheet[1:-1] if sheet.startswith("'") and sheet.endswith("'") else sheet
            )
        ...
        pattern = re.compile(
            r"('([^']+)'|"
            + "|".join(re.escape(name) for name in sheet_name_map.keys())
            + ")!"
        )
        result = re.sub(pattern, repl, formula)
        # Debug print
        print(
            f"DEBUG adjust_formula_sheet_references: input={formula}, output={result}"
        )
        return result

    @staticmethod
    def adjust_formula_sheet_references_force(
        formula: str, orig_to_final: Dict[str, str], orig_to_translated: Dict[str, str]
    ) -> str:
        """
        ...
        def repl(match):
            sheet_ref = match.group(1) or match.group(2) or ""
    ...
    @staticmethod
    def adjust_formula_sheet_references_force_precise(
        formula: str, orig_to_final: Dict[str, str]
    ) -> str:
        """
    ...
    def update_formulas_with_translated_sheetnames(wb, sheet_name_map):
        ...
        pattern = re.compile(
            r"('([^']+)'|" + "|".join(re.escape(name) for name in orig_names) + ")!"
        )
        visible_sheets = [
            sheet
            for sheet in wb.worksheets
                    ...
                    if isinstance(cell.value, str) and cell.value.startswith("="):
                        ...
                        def repl(match):
                            ...
                            sheet_unquoted = (
                                sheet_ref[1:-1]
                                if sheet_ref
                                and sheet_ref.startswith("'")
                                and sheet_ref.endswith("'")
                                else sheet_ref
                            )
                            translated = sheet_name_map.get(
                                sheet_unquoted, sheet_unquoted
                            )
                            # Quote if needed
    ...
    @staticmethod
    def replace_texts_and_sheet_names_in_xlsx(
        xlsx_path: str,
    ...
    def _translate_single(self, text: str) -> str:
        ...
        try:
            ...
            if usage:
                price_info = MODEL_PRICING.get(self.model_name, MODEL_PRICING["gpt-4o"])
                self.actual_cost += (
                    usage.get("prompt_tokens", 0) / 1000 * price_info["input"]
                ) + (usage.get("completion_tokens", 0) / 1000 * price_info["output"])
            ...
            if logger.isEnabledFor(logging.INFO):
                logger.info(
                    f"Translated: '{text[:30]}...' -> '{content.strip()[:30]}...'"
                )
    ...
    def _translate_single_threadsafe(
        self, text: str, rate_limiter: "RateLimiter"
    ) -> str:
        rate_limiter.wait()
    ...
    def translate(self):
        try:
            logger.info(
                f"Starting translation of {len(self.texts)} cells and {len(self.sheet_names)} sheet names."
            )
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = {
                    executor.submit(
                        self._translate_single_threadsafe, text, rate_limiter
                    ): idx
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = {
                    executor.submit(
                        self._translate_single_threadsafe, name, rate_limiter
                    ): idx
    ...
    def get_result(self, output_path: str) -> Optional[str]:
        if self.done and not self.error:
            replacements = {
                (sheet, cell): t if t is not None else original
                for (sheet, cell, original), t in zip(
                    self.text_cells, self.all_translations
                )
            }
            sheet_name_map = {
                old: new if new is not None else old
                for old, new in zip(self.sheet_names, self.sheet_name_translations)
            }
            self.replace_texts_and_sheet_names_in_xlsx(
                self.input_path, replacements, sheet_name_map, output_path
            )
